...
import { TabDialogo } from "./tab-dialogo";
...
const TABS = [
  ...
  { key: "dialogo", label: "Dialogo", icon: MessageCircle },
] as const;
...
export function CharacterEditorModal({ onClose }: CharacterEditorModalProps) {
  const targetId = useGameplayStore((s) => s.characterEditorTargetId);
  ...
  function updateForm(updates: Partial<CampaignCharacter>) {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          {activeTab === "dialogo" && (
            <TabDialogo form={form} onUpdate={updateForm} />
          )}
        </div>
