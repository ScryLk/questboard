...
const CHANNELS: {
  key: PlayerChatChannel;
  label: string;
  icon: typeof Hash;
}[] = [
  { key: "geral", label: "Geral", icon: Hash },
  { key: "sussurro", label: "Sussurro ao GM", icon: Lock },
];

export function PlayerChatTab() {
  const messages = usePlayerViewStore((s) => s.messages);
  const chatChannel = usePlayerViewStore((s) => s.chatChannel);
  ...
  const handleSend = useCallback(() => {
    ...
    // Add to GM store (shared)
    useGameplayStore.getState().addMessage(msg);
    // Broadcast to other tabs
    broadcastSend("player:chat", msg, "player");
  ...
  }, [input, chatChannel, playerName, myToken]);

  return (
    <div className="flex h-full flex-col">
      {/* Channel selector — only geral + sussurro (no mesa-gm) */}
...
function PlayerMessageBubble({ message }: { message: ChatMessage }) {
