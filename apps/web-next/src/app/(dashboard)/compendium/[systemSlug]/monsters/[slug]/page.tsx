...
import { ArrowLeft, Pencil, Plus, Skull, Trash2 } from "lucide-react";
...
export default function MonsterDetailPage({
  ...
  const homebrewMonster = useHomebrewStore((s) =>
    activeCampaignId
      ? s.monstersByCampaign[activeCampaignId]?.find((x) => x.slug === slug) ?? null
      : null,
  );
  const deleteMonster = useHomebrewStore((s) => s.deleteMonster);
  const isGM = useGameplayStore((s) => s.currentUserIsGM);
  const [editing, setEditing] = useState(false);

  const m = homebrewMonster ?? getMonster(systemSlug, slug);
  const isHomebrew = Boolean(homebrewMonster);
  if (!system || !m) notFound();

  function handleDelete() {
    if (!homebrewMonster || !activeCampaignId) return;
    if (
      !confirm(
        `Excluir o monstro homebrew "${homebrewMonster.name}"? Esta ação não pode ser desfeita.`,
      )
    )
      return;
    deleteMonster(activeCampaignId, homebrewMonster.slug);
    router.push(`/compendium/${systemSlug}/monsters`);
  }
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5">
          {isGM && (
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-brand-accent px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-brand-accent/80"
